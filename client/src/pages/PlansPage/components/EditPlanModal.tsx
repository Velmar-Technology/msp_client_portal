...
export function EditPlanModal({
  ...
  const closeParamPopover = () => {
    setParamDraftIndex(null);
    setParamKey('');
    setParamValue('');
  };

  const submitCustomParam = () => {
    const trimmedKey = paramKey.trim();
    if (!trimmedKey || paramDraftIndex === null) return;
    onUpdateFeatureParam?.(paramDraftIndex, trimmedKey, paramValue.trim());
    closeParamPopover();
  };

  const renderFieldError = (field: string) =>
    fieldErrors[field] ? (
      <p className="text-[10px] font-medium text-destructive">{fieldErrors[field]}</p>
    ) : null;
    ...
    <TooltipProvider>
      <Sheet open={true} onOpenChange={(open) => { if (!open) onClose(); }}>
        <SheetContent
          ...
          className="h-full p-0 flex flex-col bg-background text-foreground border-l border-border shadow-2xl overflow-hidden sm:max-w-xl md:max-w-2xl"
        >
          {/* Header */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-start">
                ...
                <div className="sm:col-span-4 space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      clearFieldError('id');
                      setEditId(e.target.value.toUpperCase().replace(/\s+/g, '-'));
                    }}
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setEditName({ ...editName, en_US: e.target.value });
                      }}
                      aria-invalid={Boolean(fieldErrors.nameEnUs)}
                      placeholder={t('plans.planNameEnPlaceholder')}
                      className="text-xs"
                    />
                    {renderFieldError('nameEnUs')}
                  </div>
                  <div className="space-y-1">
                ...
                <div className="flex items-center justify-between pb-2 mb-3 border-b border-border/60">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className="gap-1 font-semibold cursor-pointer border-zinc-200 dark:border-zinc-800"
                    >
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1.5">
                            <HintTooltip label={t('plans.dragToReorder')}>
                              <div
                                aria-label={t('plans.dragToReorder')}
                            ...
                            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              <HintTooltip label={t('plans.moveUp')}>
                                <Button
                                  ...
                                  className="text-muted-foreground disabled:opacity-20 cursor-pointer"
                                >
                            ...
                            <HintTooltip
                              label={feat.included ? t('plans.includedInPlan') : t('plans.excludedFromPlan')}
                            >
                              <Checkbox
                                checked={feat.included}
                                onCheckedChange={(checked) => onToggleFeatureIncluded(index, checked === true)}
                                aria-label={feat.included ? t('plans.includedInPlan') : t('plans.excludedFromPlan')}
                                className="size-3.5 cursor-pointer"
                              />
                            </HintTooltip>
                            ...
                            <div className="ml-6 pl-1 pt-0.5 flex items-center gap-1.5 flex-wrap">
                              {allParamKeys.map((schemaParamKey) => {
                                ...
                                return (
                                  <div
                                    key={schemaParamKey}
                                    className="flex items-center gap-1 rounded-md bg-zinc-100/70 dark:bg-zinc-900/60 px-1 py-0.5"
                                  >
                                    ...
                                    {schemaItem?.type === 'select' && schemaItem.options ? (
                                      <Select
                                        ...
                                        onValueChange={(val) => onUpdateFeatureParam?.(index, schemaParamKey, val)}
                                      >
                                      ...
                                      <Input
                                        ...
                                        className="w-16 font-mono text-foreground"
                                      />
                                    )}
                                    {!schemaKeys.has(schemaParamKey) && (
                                      <HintTooltip label={t('plans.removeParam')}>
                                        <Button
                                          ...
                                          variant="ghost"
                                          size="icon-xs"
                                          onClick={() => onDeleteFeatureParam?.(index, schemaParamKey)}
                              ...
                              >
                                ...
                                <PopoverContent align="start" className="w-56 gap-2.5 p-3">
                                  <div className="space-y-1">
                                    <Label htmlFor={`custom-param-key-${index}`} className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                                      {t('plans.paramKeyLabel')}
                                    </Label>
                                    <Input
                                  ...
                                  <div className="space-y-1">
                                    <Label htmlFor={`custom-param-value-${index}`} className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                                      {t('plans.paramValueLabel')}
                                    </Label>
                                    <Input
                          ...
                          {(!feat.code || feat.code === 'CUSTOM_FEATURE') && (
                            <div className="ml-6 pt-0.5">
                              <TabsContent value="en_US" forceMount className={featureLangTab === 'en_US' ? 'mt-0' : 'hidden'}>
                                <div className="flex items-center gap-1.5">
                                  <span className="text-[9px] font-bold text-muted-foreground uppercase w-5 shrink-0">
                                    {t('plans.langCodeEn')}
                                  </span>
                                  <Input
                                    size="sm"
