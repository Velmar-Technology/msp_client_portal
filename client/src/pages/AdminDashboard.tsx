...
export function SummaryCard({ icon, badge, title, value, subtitle, footer }: SummaryCardProps) {
  return (
    <div className="bg-white dark:bg-zinc-955 border border-zinc-205 dark:border-zinc-800 p-4 rounded-lg flex flex-col shadow-sm hover:shadow-md transition-all">
      ...
      <h3 className="text-[10px] uppercase font-bold text-zinc-400 tracking-wider">{title}</h3>
      <div className="mt-1 flex items-baseline gap-1.5">
        <span className="text-xl font-extrabold text-zinc-900 dark:text-zinc-50" style={{ fontFamily: "var(--font-heading)" }}>
          {value}
        </span>
        {subtitle && <span className="text-xs text-zinc-500">{subtitle}</span>}
      </div>
      {footer && <div className="mt-auto pt-3 border-t border-zinc-100 dark:border-zinc-900">{footer}</div>}
    </div>
  );
}
    ...
    return (
      <div className="md:col-span-4 bg-white dark:bg-zinc-955 border border-zinc-200 dark:border-zinc-800 p-4 rounded-lg flex flex-col shadow-sm min-h-[220px]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <span className="text-xs text-red-650 font-semibold">
    ...
    <div className="md:col-span-4 bg-white dark:bg-zinc-955 border border-zinc-200 dark:border-zinc-800 p-4 rounded-lg flex flex-col shadow-sm hover:shadow-md transition-all">
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex items-center gap-1.5">
          {isOffline && (
            <span className="bg-red-50 text-red-700 border border-red-200 dark:bg-red-950/20 dark:text-red-400 dark:border-red-900/50 px-1.5 py-0.5 rounded text-[9px] font-bold flex items-center gap-1 animate-pulse">
              <span className="w-1 h-1 bg-red-650 rounded-full animate-ping" />
              {t("dashboard.offline")}
            </span>
          )}
      ...
      <div className="mt-auto pt-2">
        ...
        <div className="w-full bg-zinc-100 dark:bg-zinc-900 rounded-full h-1">
          <div
    ...
    <div className="md:col-span-8 bg-white dark:bg-zinc-955 border border-zinc-200 dark:border-zinc-800 rounded-lg overflow-hidden flex flex-col shadow-sm">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-zinc-50 dark:bg-zinc-900/10 border-b border-zinc-200 dark:border-zinc-800">
              <th className="px-4 py-2 text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
          ...
          <tbody>
            {invoices.map((inv) => (
              <tr
                key={inv.id}
                className="border-b border-zinc-150 dark:border-zinc-850 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10 transition-colors h-10"
              >
                <td className="px-4 py-2 text-xs font-mono text-zinc-900 dark:text-zinc-100">{inv.invoice_number}</td>
                ...
                <td className="px-4 py-2 text-xs font-semibold text-zinc-900 dark:text-zinc-100">
            ...
            {invoices.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-6 text-center text-xs text-zinc-400 italic">
    ...
    <Page title={t("dashboard.systemOverview")} subtitle={t("dashboard.systemStatus")}>
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
        ...
        <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Support Status */}
          <SummaryCard
            icon={<Headphones className="h-5 w-5 text-zinc-650" />}
            badge={
              <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/50 px-1.5 py-0.5 rounded text-[9px] font-bold tracking-wider uppercase flex items-center gap-1">
            ...
            footer={
              <Link to="/tickets" className="text-xs text-zinc-900 dark:text-zinc-100 hover:underline flex items-center gap-1">
          ...
          {/* Maintenance */}
          <SummaryCard
            icon={<Wrench className="h-5 w-5 text-zinc-650" />}
            badge={
              <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/50 px-1.5 py-0.5 rounded text-[9px] font-bold tracking-wider uppercase">
            ...
            footer={<span className="text-[10px] text-zinc-400">{t("dashboard.preventiveNetworkReview")}</span>}
          />

          {/* Backups */}
          <SummaryCard
            icon={<CloudUpload className="h-5 w-5 text-zinc-650" />}
            badge={
              <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/50 px-1.5 py-0.5 rounded text-[9px] font-bold tracking-wider uppercase">
            ...
            footer={<span className="text-[10px] text-zinc-400">{t("dashboard.mainDbServer")}</span>}
          />
        </div>
