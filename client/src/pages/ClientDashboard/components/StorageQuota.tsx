...
export function StorageQuota({
  ...
  const usagePercentage = totalStorageQuota > 0
    ? Math.min(100, Math.round((activeStorageQuota / totalStorageQuota) * 100))
    : 0;
  ...
  return (
    <div className="bg-zinc-50/50 dark:bg-zinc-900/30 border border-zinc-200/60 dark:border-zinc-800/80 p-3.5 rounded-lg flex flex-col justify-between shadow-[0_1px_2px_rgba(0,0,0,0.02)] h-full">
      <div className="flex justify-between items-center mb-3">
        <h4 className="text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
          {t('dashboard.cloudStorage')}
        </h4>
        <Cloud className="h-4 w-4 text-zinc-400 dark:text-zinc-500" />
      </div>
