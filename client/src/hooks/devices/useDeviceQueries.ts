...
function fillEmptySlots(grouped: Record<string, Partial<SubscriptionEquipment>[]>, subs: Subscription[]) {
...
export function useDeviceQueries(isAdmin: boolean) {
  ...
  const adminQuery = useQuery({
    ...
    select: (rawDevices) => {
      ...
      // Fill holes in slot arrays
      for (const subId of Object.keys(grouped)) {
        const arr = grouped[subId];
        for (let i = 0; i < arr.length; i++) {
          if (!arr[i]) {
            arr[i] = { id: `device-slot-${i}`, subscription_id: subId, slot_index: i, status: "PENDING_ACTIVATION" };
          }
        }
      }
  ...
  const clientEquipment = useMemo(() => {
    ...
    if (!activeSubs || !allDevices) return { grouped: {} as Record<string, Partial<SubscriptionEquipment>[]>, activeSubIds: new Set<string>() };
  ...
  const loading = isAdmin ? adminQuery.isLoading : (subscriptionsQuery.isLoading || devicesQuery.isLoading);
  ...
  const activateMutation = useMutation({
    mutationFn: (data: { otp: string; subscriptionId: string; slotIndex: number; deviceName?: string; deviceSerial?: string }) =>
  ...
  const addDeviceMutation = useMutation({
    mutationFn: (data: { deviceName: string; deviceSerial?: string; tenantId?: string; otp: string }) =>
      equipmentService.addAdminDevice(data),
    onSuccess: (_result, variables) => {
      invalidate();
      toast.success(t("devices.addAdminDeviceSuccess", { name: variables.deviceName }) || "Device added successfully", {
        description: t("devices.addAdminDeviceSuccessDesc") || `Device ${variables.deviceName} has been registered and provisioned.`,
      });
    },
    onError: (err: Error) => {
      const error = err as { response?: { data?: { message?: string } }; message?: string };
