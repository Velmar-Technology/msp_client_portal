...
export const PayModal = ({ isOpen, onClose, invoice, onSuccess, t }: PayModalProps) => {
  ...
  useEffect(() => {
    ...
    async function initializePaypal() {
      ...
      const globalWin = window as unknown as { paypal?: { Buttons: (opts: unknown) => { render: (selector: string) => Promise<void>; close: () => void } } };
      if (!globalWin.paypal) {
            ...
            onApprove: async (data: { orderID: string }) => {
              ...
              try {
                ...
                if (response.success) {
                  ...
                  setPaymentMessage((response as { message?: string }).message || t('billing.paymentFailed') || 'Payment capture failed');
                }
              } catch (err) {
                console.error(err);
