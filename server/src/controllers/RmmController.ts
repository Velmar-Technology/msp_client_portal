...
export class RmmController {
  ...
  async getOverview(req: Request, res: Response): Promise<void> {
    ...
    const overview = await this.rmmPatchSvc.getRmmOverview(tenantId);
  ...
  async getEquipmentPatches(req: Request, res: Response): Promise<void> {
    ...
    const patches = await this.rmmPatchSvc.getEquipmentPatches(equipmentId, tenantId);
  ...
  async triggerScan(req: Request, res: Response): Promise<void> {
    const tenantId = (req as any).user.tenant_id;
    const equipmentId = String(req.params.equipmentId);
    const telemetry = await this.rmmPatchSvc.triggerPatchScan(equipmentId, tenantId);
  ...
  async applyPatches(req: Request, res: Response): Promise<void> {
    ...
    const applied = await this.rmmPatchSvc.applyPatches(equipmentId, patchIds || [], tenantId);
