...
1. Configurar variables en `packages/mcp-server/.env`:
   ...
   MSP_API_URL=https://helpdesk.velmartech.com.do/api/v1
...
3. Endpoints expuestos:
   - `https://helpdesk.velmartech.com.do/mcp/caf`: **Aislamiento Total.** Expone exclusivamente las 7 herramientas pedagógicas del modelo CAF y filtro de privacidad. No expone ni ejecuta herramientas de infraestructura IT (PowerShell, tickets, facturación).
   - `https://helpdesk.velmartech.com.do/mcp`: Endpoint administrativo de soporte IT para uso del equipo de Velmar (requiere `MCP_SERVER_API_KEY`).
...
```json
{
  "mcpServers": {
    "caf-quality-agent": {
      "command": "node",
      "args": ["C:/Users/PC/Workspace/msp_client_portal/packages/mcp-server/dist/index.js", "--caf"],
      "env": {
...
_Nota: Al incluir el argumento `--caf`, el servidor no requiere credenciales de la API de IT (`MSP_API_KEY`) y corre completamente aislado._
