...
export const QuickTicketModal: React.FC<QuickTicketModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim()) {
      setError(t('quickTicket.errorTitleRequired'));
      return;
    }
    if (!description.trim()) {
      setError(t('quickTicket.errorDescRequired'));
      return;
    }
    ...
    try {
      ...
      setIsSubmitting(false);
      setError(err instanceof Error ? err.message : t('quickTicket.errorGeneric'));
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              >
                ...
                <option value="SERVICE_OUTAGE">{t('common.category.serviceOutage')}</option>
                <option value="PREVENTATIVE_MAINTENANCE">{t('common.category.preventativeMaintenance')}</option>
                <option value="WARRANTY">{t('common.category.warranty')}</option>
                <option value="AI">{t('common.category.ai')}</option>
              </select>
            </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            ...
            >
              ...
              {isSubmitting ? t('quickTicket.transmittingBtn') : t('quickTicket.sendBtn')}
            </button>
          </div>
        </form>
      </div>
