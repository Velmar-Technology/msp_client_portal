    ...
    async () => {
      try {
        ...
        const psCmd = [
          ...
          'try { $tpm = Get-Tpm; $report.tpm = [PSCustomObject]@{ Present = $tpm.TpmPresent; Ready = $tpm.TpmReady; Enabled = $tpm.TpmEnabled } } catch { $report.tpm = "TPM query unavailable" }',
          'try { $report.secureBoot = Confirm-SecureBootUEFI } catch { $report.secureBoot = "SecureBoot query unavailable or Legacy BIOS" }',
          'try { $uac = Get-ItemProperty -Path "HKLM:\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System"; $report.uacEnabled = ($uac.EnableLUA -eq 1) } catch { $report.uacEnabled = $null }',
          'try { $rdp = Get-ItemProperty -Path "HKLM:\\System\\CurrentControlSet\\Control\\Terminal Server"; $report.rdpEnabled = ($rdp.fDenyTSConnections -eq 0) } catch { $report.rdpEnabled = $null }',
          'try { $report.executionPolicy = (Get-ExecutionPolicy).ToString() } catch { $report.executionPolicy = $null }',
