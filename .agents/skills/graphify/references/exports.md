...
```powershell
& (Get-Content graphify-out\.graphify_python) -m graphify export neo4j --push bolt://localhost:7687 --user neo4j --password PASSWORD
```
...
To configure in an agent config or Claude Desktop, set `command` to the **absolute interpreter path** stored in `graphify-out\.graphify_python`:
```json
{
  "mcpServers": {
    "graphify": {
